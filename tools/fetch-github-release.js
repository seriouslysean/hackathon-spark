...
async function getPRsMergedBetweenTags(owner, repo, baseTag, headTag) {
  ...
  try {
    ...
    const baseTagDate = (await client.request('GET /repos/{owner}/{repo}/git/commits/{commit_sha}', {
      owner,
      repo,
    ...
    const headTagDate = (await client.request('GET /repos/{owner}/{repo}/git/commits/{commit_sha}', {
      owner,
      repo,
...
(async () => {
  ...
  const prs = await getPRsMergedBetweenTags(GITHUB_OWNER, GITHUB_REPO, previousVersion, currentVersion);
